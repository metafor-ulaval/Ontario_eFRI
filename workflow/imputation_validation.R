...
list.files("./metrics", full.names = TRUE) %>%
  read_metrics() -> metrics_infos

metrics_infos %>%
  filter(type == "lidar" | type == "sentinel2") %>%
  filter(resolution == 20) %T>%
...
# 🟡 Sort FRI polygons based on harvest and fire ----
...
fri_polygons %<>%
  ...
  mutate_metrics(metrics, fun = "median") %>%
  mutate_prop_forested(landcover) %>%
  ...
  filter(z_p95_median >= 5, z_above2_median >= 50) %>%  # remove polygon with low vegetation according to lidar data
  mutate_species_prop("SPCOMP") %>%
  mutate_species_order("SPCOMP") %>%
  mutate_forest_type() %>%
  mutate_functional_group() %>%
  mutate_centroid()
...
fri_polygons %>%
  st_drop_geometry() %>%
  dplyr::select(any_of(paste0(metrics_names, "_median"))) %>%
...
var_order <- var_order[var_order != "tpi"]
var_order <- c(var_order, "tpi")
...
corr_matrix %>%
  ...
  mutate(Variable1 = factor(Variable1, levels = var_order),
         Variable2 = factor(Variable2, levels = var_order)) -> corr_matrix_long
...
ggsave("./imputation_results/correlation_matrix.jpg",
       width=20,
       height=20,
       dpi=300)

# Create the heatmap plot with ggplot2
ggplot(data = corr_matrix_long %>% filter(Variable1 %in% metrics_names_filtered & Variable2 %in% metrics_names_filtered),
       mapping = aes(x = Variable1, y = Variable2, fill = Correlation)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(Correlation, 2)),
            color = "black", size = 3) +
...
ggsave("./imputation_results/correlation_matrix_trim.jpg",
       width=12,
       height=12,
       dpi=300)
...
fri_polygons %>%
  st_drop_geometry() %>%
  dplyr::select(any_of(paste0(metrics_names_filtered, "_median"))) %>%
  rename_with(~str_remove(., "_median")) %>%
  drop_na -> fri_polygons_pca

metrics_infos %>%
  filter(name %in% metrics_names_filtered) %>%
  arrange(match(name, metrics_names_filtered)) %>%
  mutate(color = case_when(type == "lidar" ~ "black",
                           type == "sentinel2" ~ "red")) %>%
  pull(color) -> label_color
...
tidyterra::autoplot(pca,
                    data = fri_polygons_pca,
                    size = 0.2,
                    color = "yellow",
                    loadings = TRUE,
                    loadings.colour = label_color,
                    loadings.label = TRUE,
                    loadings.label.size = 5,
                    loadings.label.colour = label_color,
                    loadings.label.repel= TRUE,
                    variance_percentage = FALSE)

ggsave("./imputation_results/pca_trim.jpg",
       width=15,
       height=15,
       dpi=300)
...
r <- 1
target_variable_selected <- "SP_NO_1"
knn_vars <- knn_vars_combs_filtered[1]
...
results %>%
  saveRDS("./imputation_results/imputation_results.rds")
...
ggsave("./imputation_results/most_frequent_variables.jpg",
       width=20,
       height=10,
       dpi=300)
...
ggsave("./imputation_results/mean_accuracy.jpg",
       width=20,
       height=10,
       dpi=300)
...
ggsave("./imputation_results/mean_kappa.jpg",
       width=20,
       height=10,
       dpi=300)
...
ggsave("./imputation_results/mean_macro_precision.jpg",
       width=20,
       height=10,
       dpi=300)
...
ggsave("./imputation_results/mean_macro_recall.jpg",
       width=20,
       height=10,
       dpi=300)
...
ggsave("./imputation_results/mean_macro_F1.jpg",
       width=20,
       height=10,
       dpi=300)
...
results_best_model %>%
  write.csv("./imputation_results/best_model.csv")
...
ggsave("./imputation_results/random_forest_variable_importance.jpg",
       width=20,
       height=10,
       dpi=300)
